...
def func1(num: int) -> int:
    acc = 1
    non_prime_divs = [v for v in range(3, num//2+1) if num%v==0 and not(isPrime(v))]
    if not isPrime(num):
...
def func3(num: int) -> int:
    ...
    prime_divs = set(v for v in range(3, num//2+1) if num%v==0 and isPrime(v))
    if isPrime(num):
        prime_divs.add(num)
